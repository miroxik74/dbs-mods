module.exports = {
    name: "Captcha",
    author: ["PlayboyPrime#3839", "@aqusorias"],
    version: "1.0.1",
    changelog: "Changed some things that it works again with the new version of BSD -Aqusorias",
    isEvent: false,
    isResponse: true,
    isMod: true,
    isAddon: false,
    ...
    html: function (data) {
        return `
        <h3>Tabs</h3>
        <label style="float: left; margin-left: 3px; padding: 20px; background: lightgray; color: black" for="image">Image</label>
        <input style="display: none" type="radio" onchange="changetab(this.id)" checked="checked" id="image" name="tab">
        <label style="float: left; margin-left: 10px; padding: 20px; background: lightgray; color: black" id="msglabel" for="message">Message</label>
        ...
        <label style="float: left; margin-left: 10px; padding: 20px; background: lightgray; color: black" for="code">Code</label>
        <input style="margin-bottom:50px; display: none" type="radio" onchange="changetab(this.id)" id="code" name="tab">
        <label style="float: left; margin-left: 10px; padding: 20px; background: lightgray; color: black" for="awaitusermsg">AwaitUserMessage</label>
        ...
        switch (action.msgtype) {
            case "Message":
                if (action.messagechannel.trim() == "{commandchannel}") {
                    ...
                    message.guild.channels.cache.get(action.messagechannel, message.channel)
                        .send({ files: [captchaAttachment], content: action.msg });
                }
                break;
            ...
            case "Embed":
                ...
                if (action.embedchannel.trim() == "{commandchannel}") {
                    ...
                    message.guild.channels.cache.get(action.embedchannel, message.channel)
                        .send({ embeds: [embed], files: [captchaAttachment] });
                }
                break;
