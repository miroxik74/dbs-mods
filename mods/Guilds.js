...
<div class="col">
    ...
    mod: async function (DBS, message, action, args, command, index) {
        ...
        switch (action.main) {
            case "create":
                const Guild = await client.guilds.create(DBS.BetterMods.parseAction(action.guildname, message), {
                    channels: [
                        { "name": "auto-generated" },
                    ]
                })
                ...
                break
            case "icon":
                const guild = client.guilds.cache.get(DBS.BetterMods.parseAction(action.guildid, message))
                guild.setIcon(DBS.BetterMods.parseAction(action.guildicon, message))
                DBS.callNextAction(command, message, args, index + 1);
                break
            case "name":
                const guild1 = client.guilds.cache.get(DBS.BetterMods.parseAction(action.guildid, message))
                guild1.setName(DBS.BetterMods.parseAction(action.guildname, message))
                DBS.callNextAction(command, message, args, index + 1);
                break
            case "region":
                const guild2 = client.guilds.cache.get(DBS.BetterMods.parseAction(action.guildid, message))
            ...
            case "delete":
                ...
                break
            case "owner":
                const guild3 = await client.guilds.cache.get(DBS.BetterMods.parseAction(action.guildid, message))
                let owner = client.users.fetch(DBS.BetterMods.parseAction(action.guildowner, message));
