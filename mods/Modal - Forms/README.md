...
- `modalForm.js` - the form that user will get after interacting with slash command
- `modalOutput.js` - the results of what is executed after the user sent the form

3. In BSD app, create new slash command with name `modal` and Save changes

5. If downloaded, open [`AAA_BetterMods.js`](https://github.com/miroxik74/dbs-mods/blob/main/mods/AAA_BetterMods.js) and find 
```
DBS.Bot.on("interactionCreate", async interaction => {
```

6. Above the line `//interaction.followUp({ content: "hello" });`, but under `else {` , press Enter (to start new line) and paste:
```
...
7. Remember to update id of guild and channel in `modalOutput` !

8. Start your bot and use the slash command that was created in p.3