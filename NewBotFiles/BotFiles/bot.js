...
DBS.loadMods = async function () {
    let dir = path.join(__dirname, "mods");
    if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir);
    }
    require("fs")
        .readdirSync(path.join(__dirname, "mods"))
        .forEach(mod => {
            if (mod.endsWith('.js')) {
                const fetchedMod = require(path.join(__dirname, `mods/${mod}`));
                fetchedMod.init(DBS);
...
DBS.registerButtonsAndSelects = async function () {
    let buttonsAndSelects = deepSearchItems(DBS.PrefixCommandsFile.command, "rowtype", (k, v) => k === "rowtype");
...
DBS.registerSlashCommands = async function () {
    let data = [];
    DBS.SlashCommandsFile.command.forEach(command => {
